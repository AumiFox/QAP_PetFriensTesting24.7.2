...
class PetFriends:
    ...
    def get_list_of_pets(self, auth_key: json, filter: str = "") -> json:
        ...
        res = requests.get(self.base_url + 'api/pets', headers=headers, params=filter_params)
        status = res.status_code
        result = ""
        try:
            result = res.json()
    ...
    def add_new_pet_simple(self, auth_key: json, name: str, animal_type: str, age: str) -> json:
        """Добавляет питомца без фото (POST /api/create_pet_simple)"""
        headers = {'auth_key': auth_key['key']}
        data = {
            'name': name,
            'animal_type': animal_type,
    ...
    def add_photo_to_pet(self, auth_key: json, pet_id: str, pet_photo: str) -> json:
        """Добавляет фото к существующему питомцу (POST /api/pets/set_photo/{pet_id})"""
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
            })
